...
class final_topo(Topo):
  def build(self):
    ...
    server1 = self.addHost('server1', mac='00:00:00:00:00:04',ip='10.0.4.104/24',defaultRoute="server1-eth0")
    uhost = self.addHost('uhost',mac='00:00:00:00:00:10',ip='128.114.50.10/24',defaultRoute="uhost-eth0")
    ...
    self.addLink(s4,server1, port1=1, port2 =0)
    self.addLink(s5,s1, port1=1,port2=2)
    self.addLink(s5,s2, port1=2,port2=2)
    self.addLink(s5,s3, port1=3,port2=2)
    self.addLink(s5,s4, port1=4,port2=2)
    self.addLink(s5,uhost, port1=5,port2=0)
def configure():
  topo = final_topo()
 # net = Mininet(topo=topo)
  net = Mininet(topo=topo, controller=RemoteController)
  net.start()
  h1,h2,h3,server1,uhost = net.get('h1','h2','h3','server1','uhost')
  CLI(net)
